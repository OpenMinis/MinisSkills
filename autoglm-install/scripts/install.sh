...
# 在这里填入你的 API Key（获取地址：https://bigmodel.cn/）
...
echo "=============================================="
echo ""
echo "AutoGLM 需要智谱 API Key 才能运行。"
echo "获取方式："
echo "  1. 访问 https://bigmodel.cn/"
echo "  2. 注册账号并实名认证"
echo "  3. 控制台 → API Key → 创建新 Key"
echo ""
echo "配置文件已保存到：${INSTALL_DIR}/.env"
echo ""
read -p "请输入您的 API Key（可稍后在.env 中填写，直接回车跳过）: " api_key_input

if [ -n "$api_key_input" ]; then
    # 使用 sed 替换 .env 中的占位符
    sed -i "s/AUTOGLM_API_KEY=your-api-key-here/AUTOGLM_API_KEY=${api_key_input}/" "${INSTALL_DIR}/.env"
    log_info "✅ API Key 已保存！"
else
    log_warn "⚠️  稍后可编辑 ${INSTALL_DIR}/.env 文件填入 API Key"
fi

# ==============================================================================
# 创建快捷脚本
# ==============================================================================
cat > /root/aglm.sh << SCRIPT_EOF
#!/bin/bash
# AutoGLM Phone Agent 快捷脚本 - 用自然语言控制安卓手机
...
# 检查参数
if [ -z "\$1" ]; then
    echo "🤖 AutoGLM - 自然语言控制安卓手机"
    echo ""
    echo "用法：aglm.sh <任务描述>"
    echo ""
    echo "示例："
    echo "  aglm.sh '打开微信'"
    echo "  aglm.sh '打开小红书搜索美食'"
    echo "  aglm.sh '截图并保存'"
    echo ""
    exit 1
fi
...
echo "=================================================="
echo "任务：\$TASK"
echo "设备：\${DEVICE_ID:-未设置}"
echo "模型：\${AUTOGLM_MODEL:-autoglm-phone-9b}"
echo "API: \${AUTOGLM_BASE_URL:-https://open.bigmodel.cn/api/paas/v4}"
echo "=================================================="
echo ""
...
echo "   /root/aglm.sh \"打开小红书搜索美食\""
echo ""
echo "💡 常用命令示例:"
echo "   aglm.sh '检查当前屏幕'"
echo "   aglm.sh '打开抖音'"
echo "   aglm.sh '截图并保存到相册'"
echo ""
echo "🔧 修改配置（可选）:"
echo "   编辑文件：${INSTALL_DIR}/.env"
echo "   或运行命令："
...
if [ -z "$api_key_input" ]; then
    echo "⚠️  请先在 ${INSTALL_DIR}/.env 中填入 API Key！"
    echo ""
    echo "获取 API Key:"
    echo "  https://bigmodel.cn/  → 控制台 → API Key"
else
    echo "✅ API Key 已配置完成！"
fi
echo ""
...
echo "=============================================="
echo ""
echo "• API Key 未配置：编辑 ${INSTALL_DIR}/.env 填入 AUTOGLM_API_KEY"
echo "• 设备未找到：adb kill-server && adb connect 设备 IP:PORT"
echo "• 连接超时：ping 设备 IP 检查网络连通性"
echo ""
echo "=============================================="
echo ""
log_info "祝您使用愉快！🎉"
