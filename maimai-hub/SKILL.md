...
```
1. browser_use navigate → https://maimai.cn/company/gossip_discuss?webcid=9AG14xzt
2. browser_use set_user_agent → desktop_safari  （若当前是移动版）
3. browser_use get_cookies → 存入 env 文件
4. 确认 env 文件中有 COOKIE_CSRFTOKEN（否则重试步骤1-3）
```
...
2. **公司名匹配** → 脚本内置缓存 + 动态查排行榜（`circle_rank` 命令）
3. **当前用户公司** → `browser_use find_elements 'a[href*="gossip_discuss"]'` 从侧边栏提取
